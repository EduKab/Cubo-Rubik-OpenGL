...
colores = ((0, 0.274, 0.678),  # azul
           (1, 1, 1),  # blanco
           (0.0, 0.607, 0.282),  # verde
           (1, 0.835, 0),  # amarillo
           (0.7176, 0.070, 0.203),  # rojo
           (1, 0.245, 0))  # naranja


# Esta clase es para la generacion de los cuadros del cubo de Rubik.
class Cubo():
    ...
    ##
    def isAffected(self, p_x, p_y, p_direccion):
        return self.current_i[p_x] == p_y
    ...
    def m_actualizar(self, p_x, p_y, p_direccion):
        ...
        i, j = (p_x + 1) % 3, (p_x + 2) % 3
        for k in range(3):
            self.rotacion[k][i], self.rotacion[k][j] = -self.rotacion[k][j] * p_direccion, self.rotacion[k][
                i] * p_direccion

        ## error aqui
        self.current_i[i], self.current_i[j] = (
            self.current_i[j] if p_direccion < 0 else self.numeCuadros - 1 - self.current_i[j],
            self.current_i[i] if p_direccion > 0 else self.numeCuadros - 1 - self.current_i[i])
    ...
    def m_draw(self, p_colores, p_superficies, p_vertices, p_animacion, p_animAngulo, p_X, p_Y, p_direccion):
        ...
        self.colores = p_colores
        self.vestices = p_vertices
...
class Rubik():
    ...
    def mainloop(self):
        ...
        rotaVistaFlechas = {K_UP: (-1, 0), K_DOWN: (1, 0), K_LEFT: (0, -1), K_RIGHT: (0, 1)}
        ...
        anguX, anguY, rotaCubo = 45, 135, (0., 0.3)
